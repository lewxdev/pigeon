...
class AuthSignup extends Component {
  ...
  handleErrors = error => {
    if (error === "username must be unique") {
      ...
      if (error.includes("displayName")) return `Display Name ${lengthError}`;
      else if (error.includes("username")) return `Username ${lengthError}`;
      else if (error.includes("password")) return `Password ${lengthError}`;
    ...
    <Fragment>
      <Form className="SignupForm" onSubmit={this.handleSignup}>
        ...
        <Form.Field className="SignupForm_submit-options">
          <Link to="/">
            <Form.Button basic content="Back" />
          </Link>
          <Form.Button
            basic
            color="red"
      ...
      {this.props.error && (
        <p style={{ color: "red" }}>
          {this.handleErrors(this.props.error.message)}
        </p>
      )}
